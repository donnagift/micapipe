...
# Subcortex
sctx = np.loadtxt(funcDir+'/volumetric/' + subject + '_space-' + fmriType +'_desc-singleecho_timeseries_subcortical.txt')
if sctx.shape:
    n_sctx = sctx.shape[1]
else:
...
cereb_tmp = np.loadtxt(funcDir+'/volumetric/' + subject + '_space-' + fmriType +'_desc-singleecho_timeseries_cerebellum.txt')
f = open(funcDir+'/volumetric/' + subject + '_space-' + fmriType +'_desc-singleecho_cerebellum_roi_stats.txt', "rt")
cerebLabels = f.read()
...
np.savetxt(funcDir+'/surfaces/' + subject + '_' + fmriType +'_space-conte69-32k_desc-timeseries_clean.txt', data_corr, fmt='%.6f')
...
for parcellation in parcellationList_conte:
    ...
    np.savetxt(funcDir + '/surfaces/' + subject + '_' + fmriType +'_space-conte69-32k_atlas-' + parcellation.replace('_conte69','') + '_desc-FC.txt',
               ts_r, fmt='%.6f')
...
os.chdir(funcDir+'/surfaces/')
x_lh_nat = " ".join(glob.glob(funcDir+'/surfaces/' + subject + '_' + fmriType +'_space-fsnative_lh_10mm.mgh'))
...
os.chdir(funcDir+'/surfaces/')
x_rh_nat = " ".join(glob.glob(funcDir+'/surfaces/'+'*_' + fmriType +'_space-fsnative_rh_10mm.mgh'))
...
for parcellation in parcellationList:
    ...
    np.savetxt(funcDir + '/surfaces/' + subject + '_' + fmriType +'_space-fsnative_atlas-' + parcellation + '_desc-timeseries.txt', ts, fmt='%.12f')
    ...
    np.savetxt(funcDir + '/surfaces/' + subject + '_' + fmriType +'_space-fsnative_atlas-' + parcellation + '_desc-FC.txt', ts_r, fmt='%.6f')
...
plt.savefig(funcDir+'/surfaces/' + subject + '_' + fmriType +'-framewiseDisplacement.png', dpi=300)

del fd

# tSNR
lh_nat_noHP = " ".join(glob.glob(funcDir+'/surfaces/'+'*_' + fmriType +'_space-fsnative_lh_NoHP.mgh'))
lh_nat_noHP_data = nib.load(lh_nat_noHP)
lh_nat_noHP_data = np.squeeze(lh_nat_noHP_data.get_fdata())
rh_nat_noHP = " ".join(glob.glob(funcDir+'/surfaces/'+'*_' + fmriType +'_space-fsnative_rh_NoHP.mgh'))
...
np.savetxt(funcDir+'/surfaces/' + subject + '_' + fmriType +'_desc-tSNR.txt', tSNR, fmt='%.12f')
